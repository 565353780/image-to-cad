...
def back_project(xy_grid, depth, intr, invert_intr=True):
...
def depth_bbox(depth_points, depth, dummy=1e3, eps=1e-5):
    mask = depth <= eps
...
def depth_bbox_center(depth_points, depth, dummy=1e6):
    min_coords, max_coords = depth_bbox(depth_points, depth, dummy=dummy)
    return (max_coords + min_coords) / 2, min_coords, max_coords

def procrustes(nocs, depth_points, masks,
               scale=None, trans=None, mask_inputs=True,
               ret_trans=False, mask_probs=None,
               zero_center=False, prep_probs=True):
...
def point_mean(pts, num_points):
    return pts.flatten(2).sum(-1) / num_points

def point_count(masks, eps=1e-5):
    return masks.flatten(2).sum(-1).clamp(eps)

def residuals(s_nocs, it_depths,
              masks=None, rot=None,
              trans=None, hard_weights=None):
...
def irls(s_nocs, it_depths, masks,
         weights=None, num_iter=1,
         ret_history=False):
    assert num_iter >= 1
...
def transform(inputs, masks=None, scale=None,
              rot=None, trans=None, mask_outputs=True):
...
def inverse_transform(inputs, masks=None, scale=None,
                      rot=None, trans=None):
...
def reflect_x(points, x_reflection):
...
def make_new(struct, has_alignment, tensor):
