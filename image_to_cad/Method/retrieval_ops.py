...
def random_retrieval(cad_points):
    index = torch.randint(
        low=0,
        high=cad_points.size(0),
        size=(1,)
    )[0]
    distance = torch.Tensor(1.0, device=cad_points.device)
    return index, distance

def nearest_points_retrieval(points, mask, cad_points,
                             use_median=False, mask_probs=None,
                             min_points=20):
    mask = mask > 0.5
...
def mask_point_features(x, mask):
    if mask is not None and mask.numel() > 0:
        x = x * mask + x.min(-1, keepdim=True).values * mask.logical_not()
    return x

def pairwise_euclidian(x1, x2):
    return x1.square().sum(1)[:, None] - 2 * x1.mm(x2.t()) + x2.square().sum(1)

def embedding_lookup(pred_classes, noc_embeds,
                     cad_embeds_by_class, cad_ids_by_class):
...
def grid_to_point_list(nocs, masks):
    result = []
...
def voxelize_nocs(points_list, grid_size=32):
    # Create volumes
