...
class AlignmentHead(nn.Module):
    ...
    def __init__(self, num_classes, input_channels):
        ...
        shared_net = SharedMLP(input_size,
                               hidden_size=shape_code_size,
                               num_hiddens=2)

        global_net = MLP(shared_net.out_channels,
                         hidden_size=shape_code_size,
                         num_hiddens=2)
        ...
        # Init the scale head
        self.scale_head = MLP(shape_code_size,
                              3 * self.num_classes,
                              num_hiddens=2)

        self.trans_head = MLP(3 + shape_code_size,
                              3 * self.num_classes,
                              num_hiddens=1)
        ...
        self.noc_head = SharedMLP(noc_code_size,
                                  num_hiddens=2,
    ...
    def encode_shape(self, data):
        ...
        for b in data['predictions']['pool_boxes']:
            ...
            b.scale(
                data['predictions']['depth_features'].shape[-1] /
                image_size[-1],
                data['predictions']['depth_features'].shape[-2] /
                image_size[-2])
            scaled_boxes.append(b.tensor)

        data['predictions']['shape_features'] = L.roi_align(
            data['predictions']['depth_features'], scaled_boxes,
            self.output_grid_size)

        shape_code = self.shape_encoder(data['predictions']['shape_features'],
                                        data['predictions']['mask_pred'])

        data['predictions']['shape_code'] = self.shape_code_drop(shape_code)
        return data

    def crop_and_project_depth(self, xy_grid, depths, intrinsics_inv,
                               xy_grid_n, alignment_sizes):
        ...
        depth_points = back_project(xy_grid,
                                    depths,
                                    intrinsics_inv,
                                    invert_intr=False)
    ...
    def forward_roi_depth(self, data):
        ...
        if self.training:
            data['predictions']['intrinsics'] = Intrinsics.cat([
                p.gt_intrinsics
                for p in data['predictions']['alignment_instances']
            ]).tensor.inverse()
        else:
            inference_args = [{
            ...
            if data['predictions']['intrinsics'].size(0) > 1:
                data['predictions']['intrinsics'] = data['predictions'][
                    'intrinsics'].repeat_interleave(
                        data['predictions']['alignment_sizes'], dim=0)

        data['predictions']['roi_depths'], data['predictions'][
            'roi_depth_points'] = self.crop_and_project_depth(
                data['predictions']['xy_grid'], data['predictions']['depths'],
        ...
        if self.training:
            data['predictions']['roi_gt_depths'], data['predictions'][
                'roi_gt_depth_points'] = self.crop_and_project_depth(
    ...
    def forward_scale(self, data):
        if self.training:
            data['predictions']['alignment_classes'] = data['predictions'][
                'gt_classes']
        else:
            pred_classes = [
                x.pred_classes
                for x in data['predictions']['alignment_instances']
            ]
        ...
        data['predictions']['scales_pred'] = select_classes(
            ...
            self.num_classes, data['predictions']['alignment_classes'])
    ...
    def forward_trans(self, data):
        ...
        trans_code = L.cat([(depth_max - depth_min).detach(),
                            data['predictions']['shape_code']],
                           dim=-1)

        trans_offset = self.trans_head(trans_code)
        # per_category_trans:
        trans_offset = select_classes(trans_offset, self.num_classes,
    ...
    def forward_noc(self, data):
        if self.training:
            data['predictions']['depth_points'] = data['predictions'][
                'roi_mask_depth_points']
        else:
            data['predictions']['depth_points'] = data['predictions'][
                'roi_depth_points'].clone()
        ...
        data['predictions']['raw_nocs'] = self.noc_head(
    ...
    def noc_loss(self, data):
        gt_rot = Rotations(
            data['predictions']['rot_gt']).as_rotation_matrices()
        gt_nocs = inverse_transform(
            data['predictions']['roi_mask_gt_depth_points'],
            data['predictions']['mask_gt'], data['predictions']['scales_gt'],
            gt_rot.mats, data['predictions']['trans_gt'])

        data['losses']['loss_noc'] = 3 * masked_l1_loss(
            data['predictions']['nocs'],
            gt_nocs,
            torch.logical_and(data['predictions']['mask_pred'],
                              data['predictions']['mask_gt']),
            weights=data['predictions']['class_weights'])
        return data

    def prep_mask_probs(self,
                        mask_probs,
                        noc_codes,
                        nocs,
                        alignment_classes,
        ...
        new_probs = select_classes(
            ...
            self.num_classes, alignment_classes)

        mask_probs = new_probs
        return mask_probs

    def solve_proc(self, nocs, depth_points, noc_codes, alignment_classes,
                   has_enough, scale, masks, mask_probs):
        proc_masks = masks[has_enough]
        s_nocs = transform(nocs[has_enough], scale=scale[has_enough])
        mask_probs = self.prep_mask_probs(mask_probs, noc_codes, nocs,
                                          alignment_classes, has_enough)
        return irls(s_nocs,
    ...
    def forward_proc(self, data):
        ...
        if do_proc:
            ...
            if not self.training:
                data['predictions']['trans_pred'][
                    data['predictions']['proc_has_enough']] += data[
                        'predictions']['proc_solve_trs']
                rot = Rotations.from_rotation_matrices(
                    data['predictions']['proc_solve_rot']).tensor
                data['predictions']['rot_pred'] = make_new(
                    Rotations, data['predictions']['proc_has_enough'], rot)
        else:
            batch_size = data['predictions']['proc_has_enough'].numel()
            data['predictions']['rot_pred'] = Rotations.new_empty(
                batch_size, device=self.device).tensor
    ...
    def proc_loss(self, data):
        ...
        data['losses']['loss_trans_proc'] = l2_loss(
            data['predictions']['proc_solve_trs'] + data['predictions']
            ['trans_pred'].detach()[data['predictions']['proc_has_enough']],
            data['predictions']['trans_gt'][data['predictions']
                                            ['proc_has_enough']],
            weights=proc_class_weights)
        return data
